...
local function format_json(tab)
	...
	if err then
		msg.error("Formatting JSON failed:", err)
	end
	if json then
		return json
	else
...
local function parse_json(json)
	...
	if err then
		msg.error("Parsing JSON failed:", err)
	end
	if tab then
		return tab
	else
...
local tn_style = {
	...
	closest_index = (tn_style_format.closest_index):format(
		tn_palette.white,
		tn_palette.alpha_black,
		tn_palette.black,
		tn_palette.alpha_black,
		-1,
		-1
	...
	progress_text = (tn_style_format.progress_text):format(
		tn_palette.white,
		tn_palette.black,
		tn_palette.alpha_opaque,
		tn_palette.alpha_black,
		1,
		1,
		2,
...
local function set_mini_above()
	tn_osc.mini.y = (tn_osc.background.top - 12 * tn_osc.osc_scale.y)
end
local function set_mini_below()
	tn_osc.mini.y = (tn_osc.background.bottom + 12 * tn_osc.osc_scale.y)
end

local set_thumbnail_layout = {
	topbar = function()
		tn_osc.spacer.top = 0.25
		set_thumbnail_below(38.75)
		set_mini_above()
	end,
	bottombar = function()
		tn_osc.spacer.bottom = 0.25
		set_thumbnail_above(38.75)
		set_mini_below()
	end,
	box = function()
		set_thumbnail_above(15)
		set_mini_above()
	end,
	slimbox = function()
		set_thumbnail_above(12)
		set_mini_above()
	end,
	modernx = function()
		set_thumbnail_above(20)
		set_mini_below()
	end,
}

local function update_tn_osc_params(seek_y)
	local tn_state, tn_osc_stats, tn_osc, tn_style, tn_style_format =
		tn_state, tn_osc_stats, tn_osc, tn_style, tn_style_format
	...
	if
		tn_osc.scale.x_last ~= tn_osc.scale.x
		or tn_osc.scale.y_last ~= tn_osc.scale.y
		or tn_osc.w_last ~= tn_state.width
		or tn_osc.h_last ~= tn_state.height
		or tn_osc.osd.w_last ~= tn_osc.osd.w
		or tn_osc.osd.h_last ~= tn_osc.osd.h
	then
	...
	if osc_changed then
		...
		tn_style.spinner = (tn_style_format.spinner):format(
			min(tn_osc.thumbnail.w, tn_osc.thumbnail.h) * 0.6667,
			tn_osc.font_scale.x,
			tn_osc.font_scale.y
		)
		tn_style.closest_index = (tn_style_format.closest_index):format(
			tn_palette.white,
			tn_palette.alpha_black,
			tn_palette.black,
			tn_palette.alpha_black,
		...
		if tn_osc_stats.percent < 1 then
			tn_style.progress_text = (tn_style_format.progress_text):format(
				tn_palette.white,
				tn_palette.black,
				tn_palette.alpha_opaque,
				tn_palette.alpha_black,
			...
			tn_style.progress_mini = (tn_style_format.progress_mini):format(
				tn_palette.white,
				tn_palette.alpha_opaque,
				tn_osc.font_scale.x,
				tn_osc.font_scale.y
	...
	if not tn_osc.position.y then
		return
	end
	if
		(osc_changed or tn_osc.cursor.x_last ~= tn_osc.cursor.x)
		and tn_osc.osd.w_scaled >= (tn_osc.thumbnail.w + 2 * tn_osc.spacer.x)
	then
		...
		tn_osc.thumbnail.left, tn_osc.thumbnail.right =
			tn_osc.position.x - tn_osc.thumbnail.w * 0.5, tn_osc.position.x + tn_osc.thumbnail.w * 0.5
	...
	if osc_changed or tn_osc.display_progress.last ~= tn_osc.display_progress.current then
		...
		tn_osc.background.h = tn_osc.thumbnail.h
			+ (tn_osc.display_progress.current and (tn_osc.progress.h + tn_osc.spacer.y) or 0)
...
local function find_closest(seek_index, round_up)
	...
	if not (tn_thumbnails_indexed and tn_thumbnails_ready) then
		return nil, nil
	end
	local time_index = floor(seek_index * tn_state.delta)
	if tn_thumbnails_ready[time_index] then
		return seek_index + 1, tn_thumbnails_indexed[time_index]
	end
	...
	for i = 1, tn_osc_stats.total_expected do
		...
		if tn_thumbnails_ready[time_index] then
			return index + 1, tn_thumbnails_indexed[time_index]
		end
		index = seek_index + (i * -direction)
		time_index = floor(index * tn_state.delta)
		if tn_thumbnails_ready[time_index] then
...
local function show_thumbnail(seek_percent)
	if not seek_percent then
		return nil, nil
	end
	local scale, thumbnail, total_expected, ready =
		tn_osc.scale, tn_osc.thumbnail, tn_osc_stats.total_expected, tn_osc_stats.ready
	...
	if
		thumbnail.seek_index_last ~= seek_index
	...
	then
		...
		thumbnail.closest_index_last, thumbnail.total_expected_last, thumbnail.ready_last, thumbnail.seek_index_last =
			closest_index, total_expected, ready, seek_index
	end
	local x, y = floor((thumbnail.left or 0) / scale.x + 0.5), floor((thumbnail.top or 0) / scale.y + 0.5)
	if
		path
		and not (thumbnail.visible and thumbnail.x_last == x and thumbnail.y_last == y and thumbnail.path_last == path)
	then
...
local draw_progress = {
	[message.queued] = function(ass, index, block_w, block_h)
		ass:rect_cw((index - 1) * block_w, 0, index * block_w, block_h)
	end,
	[message.processing] = function(ass, index, block_w, block_h)
		ass:rect_cw((index - 1) * block_w, block_h * 0.2, index * block_w, block_h * 0.8)
	end,
	[message.failed] = function(ass, index, block_w, block_h)
		ass:rect_cw((index - 1) * block_w, block_h * 0.4, index * block_w, block_h * 0.6)
	end,
}
...
local function display_tn_osc(seek_y, seek_percent, ass)
	if
		not (
			...
			and tn_state.width
			and tn_state.height
			and tn_state.duration
			and tn_state.cache_dir
		) or not tn_osc_options.visible
	then
		hide_thumbnail()
		return
	end
	...
	local tn_osc_stats, tn_osc, tn_style, tn_style_format, ass_new, ass_rect, seek_percent =
		tn_osc_stats, tn_osc, tn_style, tn_style_format, ass_new, ass_rect, seek_percent * 0.01
	...
	ass_rect(
		...
		-tn_osc.spacer.x,
		-tn_osc.spacer.top,
		tn_osc.thumbnail.w + tn_osc.spacer.x,
		tn_osc.background.h + tn_osc.spacer.bottom
	)
	...
	ass_new(
		ass,
		tn_osc.position.x,
		tn_osc.thumbnail.top + tn_osc.thumbnail.h * 0.5,
		5,
		tn_style.spinner
			.. (tn_style_format.spinner2):format(
				spinner_color,
				spinner_alpha,
				tn_osc.background.rotation * seek_percent * 1080
			),
		tn_osc.background.text
	)
	...
	if tn_osc.display_progress.current ~= nil and not tn_osc.display_progress.current and tn_osc_stats.percent < 1 then
		ass_new(
			...
			tn_osc.mini.y,
			5,
			tn_style.progress_mini .. (tn_style_format.progress_mini2):format(tn_osc_stats.percent * -360 + 90),
			tn_osc.mini.text
		)
	end
	...
	if tn_osc.display_progress.current then
		local block_w, index =
			tn_osc_stats.total_expected > 0 and tn_state.width * tn_osc.scale.y / tn_osc_stats.total_expected or 0, 0
		if tn_thumbnails_indexed and block_w > 0 then
			...
			for time_index, status in pairs(tn_thumbnails_indexed) do
				...
				if
					index ~= closest_index
					and not tn_thumbnails_ready[time_index]
					and index <= tn_osc_stats.total_expected
					and draw_progress[status] ~= nil
				then
		...
		ass_new(
			ass,
			tn_osc.thumbnail.left + 3 * tn_osc.osc_scale.y,
			tn_osc.progress.mid,
			4,
			tn_style.progress_text,
			(tn_style_format.text_timer):format(tn_osc_stats.timer)
		)

		-- Text: Number or Index of Thumbnail
		local temp = tn_osc_stats.percent < 1 and tn_osc_stats.ready or closest_index
		local processing = tn_osc_stats.processing > 0
				and (tn_style_format.text_progress2):format(tn_osc_stats.processing)
			or ""
		ass_new(
			ass,
			tn_osc.position.x,
			...
			5,
			tn_style.progress_text,
			(tn_style_format.text_progress):format(temp and temp or 0, tn_osc_stats.total_expected) .. processing
		)

		-- Text: Percentage
		ass_new(
			ass,
			tn_osc.thumbnail.right - 3 * tn_osc.osc_scale.y,
			tn_osc.progress.mid,
			6,
			tn_style.progress_text,
			(tn_style_format.text_percent):format(min(100, tn_osc_stats.percent * 100))
		)
	end
end
...
mp.register_script_message(message.osc.update, function(json)
	...
	if new_data.state then
		...
		if tn_state.is_rotated then
			tn_state.width, tn_state.height = tn_state.height, tn_state.width
		end
		draw_cmd.w = tn_state.width
		draw_cmd.h = tn_state.height
		draw_cmd.stride = tn_state.width * 4
	end
	if new_data.osc_options then
		tn_osc_options = new_data.osc_options
	end
	if new_data.osc_stats then
		tn_osc_stats = new_data.osc_stats
		if tn_osc_options and tn_osc_options.show_progress then
			if tn_osc_options.show_progress == 0 then
				tn_osc.display_progress.current = false
			elseif tn_osc_options.show_progress == 1 then
				tn_osc.display_progress.current = tn_osc_stats.percent < 1
			else
				tn_osc.display_progress.current = true
			end
		end
		tn_style_format.text_progress = tn_osc_stats.total > 99 and text_progress_format.three_digits
			or text_progress_format.two_digits
		if tn_osc_stats.percent >= 1 then
			mp.command_native({ "script-message", message.osc.finish })
		end
	end
	if new_data.thumbnails and tn_state then
		...
		for time_string, status in pairs(new_data.thumbnails) do
			...
			tn_thumbnails_indexed[index] = ready
					and join_paths(tn_state.cache_dir, time_string) .. tn_state.cache_extension
				or status
...
local osc_styles = {
	...
	vidTitle = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs30\\q2\\fn" .. user_opts.titlefont .. "}",
...
function window_controls()
	...
	local first_geo = { x = controlbox_left - 5, y = button_y, an = 4, w = 30, h = 30 }
	local second_geo = { x = controlbox_left + 25, y = button_y, an = 4, w = 30, h = 30 }
	local third_geo = { x = controlbox_left + 55, y = button_y, an = 4, w = 30, h = 30 }
	...
	sh_area_y1 = (wc_geo.y + (wc_geo.h / 2))
		+ get_align(1 - (2 * user_opts.deadzonesize), osc_param.playresy - (wc_geo.y + (wc_geo.h / 2)), 0, 0)
	...
	if user_opts.windowcontrols_title then
		...
		lo.geometry = { x = titlebox_left + left_pad, y = wc_geo.y - 3, an = 1, w = titlebox_w, h = wc_geo.h }
...
function layout()
	...
	osc_param.video_margins.b = math.max(180, user_opts.blur_intensity) / osc_param.playresy
	...
	lo = add_layout("title")
	lo.geometry = geo
	lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.vidTitle, geo.x, geo.y - geo.h, geo.x + geo.w, geo.y)
	lo.alpha[3] = 0
	...
	if min then
		lo.geometry.x = osc_geo.w - 87 - pad
	end
...
function validate_user_opts()
	if
		...
		and user_opts.windowcontrols ~= "fullscreen_only"
...
function osc_init()
	...
	ne.content = function()
		local cache_state = state.cache_state
		if not (cache_state and cache_state["seekable-ranges"] and #cache_state["seekable-ranges"] > 0) then
		...
		return "Cache: " .. (min > 0 and string.format("%sm%02.0fs", min, sec) or string.format("%3.0fs", sec))
	end
	...
	ne.content = function()
		...
		local volicon = { osc_icons.volume_low, osc_icons.volume_med, osc_icons.volume_high, osc_icons.volume_loud }
...
if user_opts.showonstart then
	mp.register_event("file-loaded", show_osc)
end
if user_opts.showonseek then
